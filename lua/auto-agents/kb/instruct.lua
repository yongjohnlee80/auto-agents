...
local function render_block(spec, kb_root)
  ...
  vim.list_extend(lines, {
    ...
    "  required.** Read the doc's `revision:` frontmatter and compare it",
    "  with the persistent tool-root state file next to the doc:",
    "  `$(dirname \"$AUTO_AGENTS_MAILBOX_BOOTSTRAP_DOC\")/.agent-state/seen-revision`.",
    "  If the value differs, re-read the bootstrap doc end-to-end, adopt",
    "  the protocol changes, then update that seen-revision file. This",
    "  file intentionally lives next to the shared bootstrap doc, not only",
    "  under the instance-scoped `$AUTO_AGENTS_MAILBOX_DIR`.",
    ...
    "",
    "Mirror the mailbox bootstrap protocol for both todo docs:",
    "",
    "  1. `stat $AUTO_AGENTS_TODOS_BOOTSTRAP_DOC` and",
    "     `stat $AUTO_AGENTS_TODOS_CONVENTION_DOC`.",
    "  2. Read each doc's `revision:` frontmatter field.",
    "  3. Compare to your local stored value — suggested memory keys:",
    "     `feedback_todos_bootstrap_revision` (operational doc) and",
    "     `feedback_todo_handling` (per-project convention).",
    "  4. If the stored value is **missing OR differs**, re-read the",
    "     doc end-to-end, adopt directives, update your stored value.",
    "  5. If equal, you're up to date — proceed.",
    "",
    ...
    "",
    "Save the convention's `revision:` to your local project memory",
    "after ingest. Re-ingest only when the value changes.",
    "",
  })
