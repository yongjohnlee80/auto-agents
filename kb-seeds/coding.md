...
└── agents/<agent-name>/      ← agent-private operational scratch
    ...
    ├── reviews/              ← per-PR reviews (canonical home — attributed to reviewer)
...
3. Write the review at
   `agents/<your-name>/reviews/YYYY-MM-DD-<repo>-<branch>-pr<N>-review.md`.
   This is the **canonical** home for code reviews — drafts and final
   versions both live here, attributed to the reviewing agent. Do not
   publish reviews to `shared/`; a review is reviewer perspective, not
   project-shared truth. If a finding generalizes into a rule, promote
   it into `shared/conventions/` or a new `shared/adrs/` entry as a
   separate change.
4. Surface findings as: **must-fix**, **should-fix**, **nit**,
   **question**. No vague "consider X" — be concrete.
5. Append a `log.md` entry:
   `## [YYYY-MM-DD HH:MM] review | <reviewer> | <repo>#<pr>`.
